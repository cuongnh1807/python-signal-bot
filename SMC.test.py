...
def add_liquidity(fig, df, liquidity_data):
    ...
    for i in range(len(liquidity_data["Liquidity"])):
        if not np.isnan(liquidity_data["Liquidity"][i]):
            ...
            print(f"\nLiquidity Found at index {i}:")
            print(f"Direction: {'Bullish' if liquidity_data['Liquidity'][i] == 1 else 'Bearish'}")
            print(f"Level: {liquidity_data['Level'][i]:.2f}")
            print(f"End Index: {int(liquidity_data['End'][i])}")
            print(f"Time Range: {df.index[i]} to {df.index[int(liquidity_data['End'][i])]}")
            ...
            if liquidity_data["Swept"][i] != 0 and not np.isnan(liquidity_data["Swept"][i]):
                ...
                print(f"Swept at index {int(liquidity_data['Swept'][i])}")
                print(f"Swept Price: {swept_price:.2f}")
                print(f"Swept Time: {df.index[int(liquidity_data['Swept'][i])]}")
            print("-" * 50)
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    name=f"{'Bullish' if liquidity_data['Liquidity'][i] == 1 else 'Bearish'} Liquidity"
                )
            )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="text",
                    text=f"LIQ {liquidity_data['Level'][i]:.0f}",
...
def import_data(symbol, start_str, timeframe):
    ...
    df.index = pd.to_datetime(
        df.index, unit="ms").strftime("%Y-%m-%d %H:%M:%S")
    return df


df = import_data("BTCUSDT", "2024-11-27", "15m")
df = df.iloc[-500:]
