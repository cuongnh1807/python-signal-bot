...
class smc:
    ...
    def ob(
        ...
        # Calculate OB volume with more precision
        # Calculate volume percentage relative to recent volume
        lookback = 20  # Look back period for volume comparison
        volume_ma = pd.Series(_volume).rolling(lookback).mean()
        percentage = np.zeros(len(ohlc), dtype=np.float32)

        for i in range(len(ob)):
            if ob[i] != 0 and i >= lookback:
                # Calculate volume strength as percentage above average
                if volume_ma[i] > 0:  # Avoid division by zero
                    percentage[i] = min(
                        int((obVolume[i] / volume_ma[i]) * 100), 100)
                else:
                    percentage[i] = 0
    ...
    @ classmethod
    def pivot_points(cls, ohlc: DataFrame, method='traditional', timeframe='1d', levels=5) -> DataFrame:
        """
        Calculate Pivot Points with multiple methods and timeframes

        Parameters:
        - method: str
            'traditional': Traditional pivot points
        ...
        - timeframe: str
            ...
            '12M': Yearly
        - levels: int
            Number of support/resistance levels (1-5)
        """
        data = {}
        ...
        # Get the resampling frequency
        freq = tf_map.get(timeframe, '1D')

        # For 15-minute timeframe, we'll use a rolling window instead of resampling
        if timeframe == 'm15':
            window_size = 15
            resampled = pd.DataFrame({
                'open': ohlc['open'].rolling(window=window_size, min_periods=1).apply(lambda x: x[0]),
                'high': ohlc['high'].rolling(window=window_size, min_periods=1).max(),
                'low': ohlc['low'].rolling(window=window_size, min_periods=1).min(),
                'close': ohlc['close'].rolling(window=window_size, min_periods=1).apply(lambda x: x[-1])
            }, index=ohlc.index)
        else:
        ...
        if method.lower() == 'traditional':
            ...
            # Calculate R1-R5 and S1-S5
            data['r1'] = (2 * data['pivot']) - low
            data['s1'] = (2 * data['pivot']) - high

            data['r2'] = data['pivot'] + (high - low)
            data['s2'] = data['pivot'] - (high - low)

            data['r3'] = data['pivot'] + 2 * (high - low)
            data['s3'] = data['pivot'] - 2 * (high - low)

            data['r4'] = data['pivot'] + 3 * (high - low)
            data['s4'] = data['pivot'] - 3 * (high - low)

            data['r5'] = data['pivot'] + 4 * (high - low)
            data['s5'] = data['pivot'] - 4 * (high - low)
            ...
            data['r1'] = data['pivot'] + 0.382 * price_range
            data['s1'] = data['pivot'] - 0.382 * price_range

            data['r2'] = data['pivot'] + 0.618 * price_range
            data['s2'] = data['pivot'] - 0.618 * price_range

            data['r3'] = data['pivot'] + 1.000 * price_range
            data['s3'] = data['pivot'] - 1.000 * price_range

        elif method.lower() == 'woodie':
            # Woodie's Pivot Points
            data['pivot'] = (high + low + 2 * close) / 4
            ...
            data['r3'] = high + 2 * (data['pivot'] - low)
            data['s3'] = low - 2 * (high - data['pivot'])

            data['r4'] = data['r3'] + high - low
            data['s4'] = data['s3'] - (high - low)

        elif method.lower() == 'demark':
            # DeMark's Pivot Points
            x = np.where(close < open, high + 2*low + close,
                         np.where(close > open, 2*high + low + close,
                                  high + low + 2*close))
            ...
            data['r1'] = close + price_range * 1.1/12
            data['s1'] = close - price_range * 1.1/12

            data['r2'] = close + price_range * 1.1/6
            data['s2'] = close - price_range * 1.1/6

            data['r3'] = close + price_range * 1.1/4
            data['s3'] = close - price_range * 1.1/4

            data['r4'] = close + price_range * 1.1/2
            data['s4'] = close - price_range * 1.1/2

            data['r5'] = close + price_range * 1.1
            data['s5'] = close - price_range * 1.1

        else:
            raise ValueError("Invalid pivot point method")

        # Limit the number of levels based on input
        columns = ['pivot']
